...
function(
        sl_target_include_system_library
        target
        scope
        lib)
    ...
    if(TARGET ${lib})
        ...
        if(lib_include_dirs)
            sl_target_include_system_directories(${target} ${scope} ${lib_include_dirs})
        else()
...
function(
        sl_target_link_system_library
        target
        scope
        lib)
    # Include the directories in the library
    sl_target_include_system_library(${target} ${scope} ${lib})
...
function(sl_target_link_system_libraries target)
    ...
    foreach(scope IN ITEMS INTERFACE PUBLIC PRIVATE)
        foreach(lib IN LISTS ARG_${scope})
            sl_target_link_system_library(${target} ${scope} ${lib})
        endforeach()
    endforeach()
endfunction()
